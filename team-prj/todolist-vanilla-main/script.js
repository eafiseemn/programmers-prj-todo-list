...
/* ------------ 0610 수정: todoForm 별도로 정의 안함 > document 로 변경 필요 ----------------- */
// const inputWrap = todoForm.getElementsByClassName("input_wrap")[0];
const inputWrap = document.querySelector(".input_wrap");
/* ------------ 0610 수정: 고정 요소를 updateCount() 할 때마다 querySelector로 찾고 있으므로 전역 변수로 한 번만 실행  ----------------- */
...
/* ------------ 0610 수정: 함수 호출 시 이미 false를 전달하기 때문에 default value 설정 필요 없음  ----------------- */
...
/* 추가한 값 배열에 저장 */
/* ------------ 0610 수정: 함수 호출 시 이미 isChecked = false를 전달하기 때문에 default value 설정 필요 없음  ----------------- */
/* ------------ 0610 수정: 함수 호출 시 createdAt을 전달하는데 매개변수로 받고있지 않음 -> 추가  -------------------------------- */
...
function createErrorMsg() {
    ...
    /* ------------ 0610 수정: 전역변수로 선언했으므로 삭제 ---------------------------- */
      // const countWrap = document.querySelector("#todoCount");
      inputWrap.insertBefore(errorMsg,countWrap);
  }
} 

/* ------------ 0610 수정: 현재 시간 생성 함수 분리 및 표시 형식 변경 ---------------------------- */
...
function updateCount(){
    ...
    /* ------------ 0610 수정: todoListArray.length === 0 || 조건은 위에서 걸러지므로 중복 삭제 ---------------------------- */
